...
import { Flex, Select, Box, Input, Spinner, Icon, Button, Text } from '@chakra-ui/react';
...
const SearchFilters = () => {
  ...
  useEffect(() => {
    if (searchTerm !== '') {
      const fetchData = async () => {
        setLoading(true);
        const data = await fetchApi(`${baseUrl}/auto-complete?=${searchTerm}`);
        setLoading(false);
  ...
  return (
    <Flex flexWrap='wrap' justifyContent='center' bg='gray.100' p='4' >
      ...
      <Flex flexDir='column' >
        <Button onClick={() => setShowLocations(!showLocations)} bg='black' color='teal.400' marginTop='2' >
          Search Location
        </Button>
        {showLocations && (
